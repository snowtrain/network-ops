...
from modules.devnet_0_cpumem_interval import cpu_max_interval, mem_max_interval, cpu_interval, mem_interval, speed_interval, utilization_interval
from devnet2019.views.devnet_home_data import health_reachable, health_cpu, health_mem


def home(request):
    health_reachable(request)
    health_cpu(request)
    health_mem(request)
    devices_cpu_list = []
    devices_mem_list = []
    ...
    for device in Devicedb.objects.all():
        try:
            ...
            # 当前最新 CPU 使用率
            cpu_current = device.cpu_usage.filter(record_datetime__gt=datetime.now() - timedelta(hours=cpu_interval)).order_by('-id')[0].cpu_usage
        ...
        try:
            ...
            # 当前最新 MEM 使用率
            mem_current = device.mem_usage.filter(record_datetime__gt=datetime.now() - timedelta(hours=mem_interval)).order_by('-id')[0].mem_usage
    ...
    for interface in active_inteface:
        # 接口入向利用率监控周期内的数据
        interface_in_data = interface.interface_in_bytes.filter(record_datetime__gt=datetime.now() - timedelta(hours=utilization_interval))
        ...
        # 接口出向利用率监控周期内的数据
        interface_out_data = interface.interface_out_bytes.filter(record_datetime__gt=datetime.now() - timedelta(hours=utilization_interval))
        ...
        # 计算出向接口利用率列表
        utilization_out_data = list(map(lambda y: round(((y[0] * 8) / y[1] / interface_speed) * 100, 2), zip_out_list))
        try:
            # 向top列表增加数据
    ...
    # 以 rx_max 对列表排序并取反（即倒序）
    top_5_if_utilization_rx_dict = sorted(top_5_if_utilization_rx_dict, key=lambda y: y['rx_max'], reverse=True)
    top_5_if_utilization_rx_dict = top_5_if_utilization_rx_dict[:5]

    # 以 tx_max 对列表排序并取反（即倒序）
    top_5_if_utilization_tx_dict = sorted(top_5_if_utilization_tx_dict, key=lambda y: y['tx_max'], reverse=True)
    top_5_if_utilization_tx_dict = top_5_if_utilization_tx_dict[:3]
